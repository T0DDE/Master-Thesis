...
\begin{itemize}
    ...
    \item As previously discussed, Galactic binaries are expected to impact LISA’s sensitivity curve, generating a confusion foreground that significantly impacts the LISA sensitivity curve, primarily due to their proximity and large number (\cite{osti_6282329},~\cite{Nelemans_2001},~\cite{LISA_ref}).
\end{itemize}  
The combination of their abundance, characteristic signal strength, and astrophysical importance makes DWDs an optimal and compelling target for LISA’s mission.
The aim of this work is to determine whether \textit{extra-galactic} DWDs, despite their much greater distances, can contribute significantly to an overall detectable stochastic GW foreground in the LISA band.
