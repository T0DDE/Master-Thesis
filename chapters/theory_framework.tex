...
In this chapter we will cover all the theoretical foundations for this thesis. 
We will start by describing the astrophysics of DWDs and the galaxies hosting them.
Next, we will go through the GW theory, starting from the linearization of Einstein's equations, up to the characteristic strain that we would expect from the DWDs.
Finally, we will understand the functioning of GW detectors, using the Michelson interferometer as example, and introducing LISA's characteristics.


\section{Double White Dwarfs {(DWDs)}}
In this section, we will introduce the main astrophysical theory elements for the GW sources we want to simulate.
First of all, we focus on white dwarfs, the compact remnants of low- and intermediate-mass stars, both as isolated objects and as components of binary systems.  
...
A clear understanding of both the compact objects themselves and their host galaxies is necessary for predicting the GW signals that can be observed.

\subsection{White dwarfs}
During its evolution, a star generates energy through the nuclear fusion reactions, ``burning'' the elements in its core. 
...
For most stars, this process begins with the fusion of hydrogen into helium, and when the core runs out of hydrogen, it contracts while the external layers expand: the luminosity is now spread in a larger surface, and since the \textit{black body energy density} is $\propto T^4$, where $T$ is the surface temperature, the star will appear colder and larger, as a \textit{red giant} ({\cite{on_red_giant_phase}}).
If the star is sufficiently massive, core contraction raises the temperature enough to ignite helium fusion into heavier elements, leading to subsequent burning stages ({\cite{Hoyle:1954zz},~\cite{RevModPhys.29.547}}).  
In contrast, for medium or low massive stars ($M_*\le 8M_\odot$), after the red giant phase the central pressure is initially insufficient to ignite heavier fusion, and the contraction continues until the \textit{electron degeneracy pressure} (a quantum mechanical effect arising from the Pauli exclusion principle) halts it ({\cite{Bradt},~\cite{Chandrasekhar}}).
...
This positive feedback process induces a very fast helium burning called \textit{helium flash}, that lifts the star back out of the degeneracy stage ({\cite{on_helium_flash}}).
...
These types of medium-to-low massive star's remnant, supported entirely by electron degeneracy pressure and slowly cooling over time, are what we call \textbf{white dwarfs} ({\cite{Bradt}}).

\subsubsection{Binary systems}
A great fraction of the white dwarfs in the universe are found DWDs systems, that show a wide range of orbital separations and periods, depending on their formation and evolutionary history ({\cite{evolution_compact_binary_wd_systems}}).  
In particular, in the Milky Way, DWDs are expected to be extremely common systems, many of which having orbital periods short enough to emit GWs in the milliHertz band ({\cite{Nelemans_2001}}).  
Among the most relevant parameters for DWDs, typical values are:
\begin{itemize}
...
The nature of this transfer depends on the mass ratio: the transfer can be stable, leading to long-lived systems characterized by persistent mass accretion and bright electromagnetic counterparts, or it can become dynamically unstable, potentially resulting in the rapid merger of the two stars, which may produce a single more massive white dwarf, a neutron star, or even trigger a thermonuclear explosion if the critical conditions for a Type Ia supernova are met ({\cite{Das2025},~\cite{Shen_2009},~\cite{refId0},~\cite{Shen_2018}}).


\subsection{{The host galaxies}}
...
\begin{itemize}
    ...
    \item \textbf{Spiral galaxies (Sa, Sb, Sc, Sd):} Such as our own, they are disk galaxies with spiral arms rooted in a central bulge.
...
This metallicity fingerprint that distinguishes morphological types will be an important feature in the next chapters, where the stellar population properties will influence the compact binary formation and GW emission.

\section{Gravitational Wave Theory}
{The DWDs described in the previous section are massive rotating objects, and therefore they produce ripples in spacetime that propagate as waves (\cite{Einstein_1918}).
The purpose of this thesis is to analyze the GW produced by such systems, and it is thus necessary now to make a summary of the GW theory as a whole, to derive the characteristic strain that we will use to simulate the total signal.}
...
The typical approach to the study of GWs is to derive them as small perturbations of the foreground from the Einstein's equations: 
\begin{equation}
    G_{\mu\nu} = R_{\mu\nu} - \frac{1}{2}g_{\mu\nu}R,
\end{equation}
...
\]
where $\mathbf{k}$ is the wave three-vector orthogonal to the wavefront, and is related to the \textit{wavelength} by $|\mathbf{k}| = 2\pi/\lambda$.
...
Thus, a plane GW propagating along the $x$-axis is characterized by only two non-zero functions $h_{zy}$ and $h_{yy}=-h_{zz}$:
\begin{equation}
    h^{TT}_{\mu\nu} =  
    \begin{pmatrix}
0 & 0 & 0 & 0 \\
...
\end{equation}
The GW only has \textbf{two physical degrees of freedom} which correspond to two polarization states.
...
By imposing the rest condition, $u^\alpha = (1,0,0,0)$, to the~\eqref{eq: geodesic equation of free particle} and considering the TT gauge component prescriptions for $h_{\mu\nu}$, we find that a particle initially at rest is not accelerated by the passage of a wave, but remains at a fixed coordinate position: the motion of a single particle is not affected by the GWs.
...
Although, as we have seen, the coordinates of the two particles (and thus their difference) do not change when a GW passes, this does not hold true for their \textit{proper distance}
\[
    \Delta l = \int ds \not = const.
\]
...
As previously discussed, the non-zero components of the strain tensor $h_{ik}^{TT}$ correspond to two possible polarizations of the GW.\@
...
\begin{figure}[h]
    ...
    \end{tikzpicture}
    \caption{Effect of the two GW polarizations on a ring of free-falling particles, caused by a wave moving in the direction perpendicular to the page: blue and red dots represent two opposite phases of the wave.}\label{fig:gw_polarizations}
\end{figure}

\subsection{The quadrupole approximation}
We have discussed the effects of the passage of a GW on the relative motion of close particles; in this section we focus on another important problem: \textit{how such waves are generated}.
...
But not every motion of matter can produce GW signal, as we will see: conservation laws of general relativity, impose strict constraints on the possible multipole moment that can contribute to the tensorial metric perturbations.
...
If we assume that the region, of size $\epsilon$, where the source of the signal is confined, is much smaller than the wavelength of the emitted gravitational radiation, $\lambda_{GW} = \frac{2\pi c}{\omega}$, we find the following condition:
\begin{equation}
...
If we now use the solution found using the retarded potentials in~\eqref{eq: solutions with retarded potentials}, by making use of the Fourier transforms it can be found that the GW signal emitted by the source, to leading order in the weak-field, slow-motion approximation is
\begin{equation}
...
\begin{equation}
    \frac{1}{c^2}\frac{\partial^2}{\partial t^2}\int_V T^{00}x^k x^n d^3x = 2\int_V T^{kn}d^3x, \hspace{3mm} k,n=1,2,3,
    \notag
\end{equation}
in the definition of the \textbf{quadrupole moment tensor}
\begin{equation}
    q^{kn}(t) = \frac{1}{c^2}\int_V T^{00}(t,\mathbf{x})x^k x^n d^3x,
    \notag
\end{equation}
...
This formulation describes the GW signal emitted by a gravitating system that evolves in time, whatever the mass-energy distribution looks like, as long as $\ddot{q}^{ik}\not = 0$.
...
As we will see, to compute the ``luminosity'' of a GW source, it is useful to define the \textbf{reduced quadrupole moment}
\begin{equation}
...
\begin{align}
    ...
    &    A_{kl}(t) =  
    \begin{pmatrix}
        \cos{2\omega_K t} & \sin{2\omega_K t} & 0 \\
     \sin{2\omega_K t} &  -\cos{2\omega_K t}& 0 \\
     0 & 0 & 0 \\
    \end{pmatrix}.
...
\end{equation}
as an estimate of a typical GW signal amplitude generated by a binary system.
...
In particular, it is possible to define a \textit{local} notion of energy and momentum in the weak field approximation, on the assumption that the characteristic length-scale $\lambda$ of the perturbation is much smaller than the characteristic length-scale $L$ of the foreground: $\lambda/L\ll 1$.
...
Similarly, the energy flux of a GW propagating in the same direction is given by the same component of the stress-energy pseudo-tensor averaged over several wavelengths, and can be derived as
\[
...
\]
and writing $Q^{TT}$ as a function of~\eqref{eq: reduced quadrupole moment}, by using the TT projectors in~\eqref{eq: transverse-traceless projector} and their properties, it can be shown that the energy associated to a GW can be written as the \textbf{luminosity quadrupole formula}:
\begin{equation}
...
By applying what we have found in the previous subsection to real astrophysical sources of GW, in this section we will find the amplitude that we are going to use for this thesis work to generate the total GW signal.
In particular, we can apply the general results we found for two objects in a circular orbit in~\eqref{eq: A definition} to compute the quadratic sum of the components of $\dddot{Q}_{ij}$ in~\eqref{eq: luminosity quadrupole formula}, and find the GW luminosity of a compact binary system as
\[
...
The energy emitted in GW comes from the total energy of the system, which is in part kinetic and in part potential energy. 
...
Knowing how the orbital parameters evolve with time allows us to find the GW signal for an \textit{inspiralling compact object}.
...
\end{equation}
is the wave amplitude that we are going to refer to in this thesis work, when we calculate the signal from the simulated binaries.

\section{Gravitational Wave Interferometers}
The typical GW amplitude for a binary system is very small, and this makes it quite a hard task to detect it.
The most sensitive instruments capable of such measurements are the interferometric detectors, that are generally based on the same principle of the simple Michelson interferometer: by splitting a laser beam along two perpendicular arms and recombining it, they detect the small phase shifts induced by a passing GW.\@  
...
After this, we will talk briefly about LISA's characteristics, and what makes it interesting for the purpose of this thesis.
...
\begin{figure}
    ...
    The beams reflected from the mirrors are recombined at the detector (screen), where, if a GW has passed, they produce an interference pattern.
    Variations in the optical path length of one arm (through the movable mirror) shift the observed fringes.
The source for this image can be found \underline{\textit{\href{https://www.britannica.com/technology/Michelson-interferometer}{here}}}.}\label{fig: michaelson interferometer}
\end{figure}
...
\begin{equation}
    \left\{
        \begin{aligned}
            &E_1 = -\frac{1}{2}E_0e^{-i\omega_L t + 2ik_L L_A} \\
            &E_2 = -\frac{1}{2}E_0e^{-i\omega_L t + 2ik_L L_B},
        \end{aligned}
    \right.
...
\end{equation*}
therefore the detector measures a value depending on the cosine, which in turn depends on $\Delta L$: if there is no GW, a null measure should be expected.

\subsubsection{Gravitational waves and TT gauge}
The TT gauge describes the position of \textit{free falling} objects, that does not change when a GW passes. 
...
This apparent problem is solved when considering that the effects of GWs are perpendicular to their direction of motion, and therefore the mirrors need to be decoupled in the horizontal plane, rather than the vertical.
...
Assuming a \textit{plus-polarized} GW moving in the $z$ direction, perpendicular to the plane defined by the two arms, we have 
\[
...
\]
Therefore, when the period of the GW is large with respect to the time it takes the light to travel along the arm, the $\Delta t$ shift in the travel time $t_2 - t_0$, with respect to the unperturbed case is simply $h(t_1)L_A/c$.
...
Now, from~\eqref{eq: time interval for x-axis} and its $y$-axis counterpart, it is possible to find the two different starting times\footnote{Note that if $L_A=L_B=L$, and $\frac{\omega L}{c}\ll 1$, the difference between the two would be
\[
...
The strategy to build a ``null instrument'', which records zero signal when the GW is not passing, by setting $L_A = L_B \to \phi_0=0$, turns out to be failing, since it makes the instrument insensitive to calibration uncertainties.
...
\subsection{{The} Laser Interferometer Space Antenna ({LISA})}
{Currently operating} detectors such as LIGO and Virgo ({\cite{PhysRevLett.116.061102},~\cite{Abbott_2019}}) have pioneered the technique we have just shown in the high-frequency band ({\cite{LISA_ref},~\cite{Virgo_cit}}).
The purpose for future space-based interferometers such as LISA is to extend it to the low-frequency regime ({approximately} within $0.1$mHz and $1$Hz), opening a new observational window on astrophysical and cosmological sources.
Indeed, space-based detectors such as LISA will be sensitive to DWDs with periods of a few hours down to a few minutes ({\cite{baker2019laserinterferometerspaceantenna}}). 
...
Between the binary systems, the interacting ones can have extremely short orbital periods ($\lesssim 10$ minutes) and are strong, nearly monochromatic GW sources over LISA's lifetime, which in this work is considered of $\sim4$yr ({\cite{baker2019laserinterferometerspaceantenna}}).  
Detached systems with periods in the mHz range are also important, as they provide long-lived, stable signals that can serve as calibration sources or ``verification binaries'' {(\cite{kupfer_2018},~\cite{Kupfer_2024})} for space-based detectors.
Given their abundance and GW luminosity, binary white dwarfs are expected to be the most numerous class of resolvable sources for LISA, and their unresolved population will form a significant component of the low-frequency GW foreground.
...
In LISA's case, the arms are not going to be physical structures, but $2.5\times10^6$km separations between three identical spacecrafts at the vertices of an equilateral triangle ({\cite{baker2019laserinterferometerspaceantenna}}).
...
The spacecrafts will trail by about $20^{\circ}$, with the triangular constellation inclined by $60^{\circ}$ to the ecliptic plane, following the earth around the sun ({\cite{baker2019laserinterferometerspaceantenna}}).
...
LISA's frequency band roughly covers the low range $0.1~\mathrm{mHz} \lesssim f \lesssim 1~\mathrm{Hz}$ (compared to LIGO's frequency of 10 Hz to 10 kHz, {\cite{Martynov_2016}}), corresponding to objects in much wider orbits and potentially much heavier than those that LIGO is searching for, opening up the to the detection of a wider range of GW sources. 
In this frequency range also reside most DWDs.  
In the low frequencies, the range is limited by the acceleration noise on the test masses: even in space, the ``free'' test masses are not completely isolated, but could be influenced by solar radiation and winds, and the ``drag-free'' micro-propellers that compensate these disturbances make noise under the $0.1~\mathrm{mHz}$ ({\cite{baker2019laserinterferometerspaceantenna}}). 
...
At the high frequencies, the main limits are due to the arms length, which make LISA unable to detect waves much shorter than that scale, and the power limits of the laser, which makes the photon count Poissonian noise more relevant at high frequencies ({\cite{baker2019laserinterferometerspaceantenna}}).  
LISA’s sensitivity curve combines instrumental noise and the astrophysical foreground, including Galactic foreground and Galactic binary systems, as shown in \textbf{Figure~\ref{fig: LISA sens curve with noises}} ({\cite{LISA_ref}}). 
\begin{figure}
    \begin{center}
    ...
    The solid green line shows the instrumental noise curve, while the dashed black line represents the LISA curve as shaped by the foreground contribution from unresolved Galactic binaries.
    Different astrophysical sources that emit GW signal in this band are indicated: massive black hole binaries (MBHBs), extreme mass-ratio inspiral (EMRI), stellar-mass black hole binaries, verification binaries, and Galactic binaries with signal-to-noise ratio larger than 7.
The source for this image can be found \underline{\textit{\href{https://ar5iv.labs.arxiv.org/html/1803.01944}{here}}} ({\cite{LISA_ref}}).}\label{fig: LISA sens curve with noises}
\end{figure}
As far as the frequency resolution\footnote{Not to be confused with the sensitivity of the instrument, which states how faint of a signal it can detect.} of a coherent observation is concerned, the limits are not due to hardware, but the actual total observational time: in a Fourier analysis, this is set by the inverse of the total observation time:  
\begin{equation}
...
\subsubsection{Why double white dwarfs}
{From a scientific perspective, double white dwarfs (DWDs) provide an excellent laboratory for gravitational-wave studies:  
\begin{itemize}
    \item DWDs are typically found in compact orbits, with corresponding GW frequencies that lie within LISA's observational band;  
    \item They are among the most common compact binary systems in the universe, often representing a typical outcome of stellar binary evolution. When a large number of such sources cannot be individually resolved, their signals overlap stochastically, producing a foreground;  
    \item In the circular orbit approximation, their strain can be described by a simple and well-defined expression [Eq.~\eqref{eq: the strain we use}], which can be readily simulated and used to generate coherent synthetic populations within astrophysical models;  
    \item As previously discussed, Galactic binaries are already expected to impact LISA’s sensitivity curve, making extra-galactic populations a realistic and relevant candidate to investigate.  
\end{itemize}  
Galactic binaries are therefore expected to generate a confusion foreground that significantly impacts the LISA sensitivity curve, primarily due to their proximity and large number (\cite{osti_6282329},~\cite{Nelemans_2001},~\cite{LISA_ref}).  
The aim of this work, however, is to determine whether \textit{extra-galactic} DWDs, despite their much greater distances, can contribute significantly to an overall detectable stochastic GW foreground in the LISA band.  
The combination of their abundance, characteristic signal strength, and astrophysical importance makes extra-galactic DWDs an optimal and compelling target for LISA’s mission.  
}
% From a scientific perspective, DWDs offer a rich laboratory:  
%     \begin{itemize}
%         \item The DWDs typically are in compact orbits, and the corresponding frequencies fall in LISA's band; 
%         \item The DWDs are some of the most common compact systems in the universe, that often represent a output for binary system evolution. 
%         When the number of non-resolvable sources is large, their signals superpose stochastically, generating a foreground; 
%         \item We have found a simple and predictable impact for their signal in the circular orbit approximation, in~\eqref{eq: the strain we use}, which is possible to simulate, and allows to generate coherent synthetic populations with astrophysical models; 
%         \item As discussed previously, we already expect Galactic binaries to affect LISA's curve, therefore extra-galactic ones are a realistic candidate to test;
% \end{itemize}
% We expect the Galactic binaries to generate a confusion foreground that is able to impact the LISA sensitivity curve, mostly due to their proximity and large number {(\cite{osti_6282329},~\cite{Nelemans_2001},~\cite{LISA_ref}}.
% The purpose of this work, however, is to discover if the \textit{extra-galactic} DWDs, despite their much larger distance, {contribute significantly} to generate an overall detectable stochastic GW foreground in the LISA band.
% The combination of abundance, signal strength, and astrophysical relevance makes them an optimal target for LISA's mission.
