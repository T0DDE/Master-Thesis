...
Before computing the total extragalactic DWDs gravitational foreground, we must ensure that our list of galaxies is truly representative of the total population of DWDs in the nearby universe. 
This means we must consider that the final catalog that we use to compute the signal is incomplete, analysing all the causes of lack of galaxies.
...
Since the GWGC is an observation-based catalog derived from optical surveys, we must account for the ZOA, the region of the sky obscured by the Milky Way’s interstellar dust and stellar crowding, which impedes the detection of foreground galaxies.
...
\begin{figure}
    ...
    To estimate this ZOA, we calculated the percentage of empty pixels in the band $|b| \leq 15^\circ$ in Galactic latitude.
...
\end{figure}
To quantify its extent, we used a Python package that handles pixelated data on the sphere based on the Hierarchical Equal Area isoLatitude Pixelization, \underline{\textbf{\href{https://sourceforge.net/projects/healpix/}{HEALPix}}}\footnote{\cite{HEALPix}}, called healpy, to divide the sky map into $n_{\rm pix}$ equal-area pixels, and focused on the band $|b| \leq 15^\circ$ in Galactic latitude.
...
Before scaling up the fixed population, and calculating each binary's GW signal, and confronting it with LISA's sensitivity curve, we must then consider LISA's limited \textit{frequency resolution}, and understand how to process the signals from indistinguishable sources.

\subsection{LISA's frequency resolution}
The duration of LISA’s mission directly impacts its frequency resolution: the longer the observation time, the finer the binning in frequency.
This must not be confused with the sensitivity of the instrument, which tells how low of a GW strain it is able to detect.
...
Although this is a very fine resolution, the precision of the orbital period of the simulated binary systems, and the corresponding GW frequency resolution, is so big that we must consider the fact that LISA will not be able to distinguish sources whose frequencies fall within the same frequency bin.
...
In order to compute the total extragalactic DWDs gravitational foreground, we must apply an iterative process to each galaxy in the final catalog, which consists of the following steps:
\begin{itemize}
...
\section{Discussion}
The main result of this work is the spectral distribution of the final computed signal plotted over the LISA's sensitivity curve\footnote{This particular curve is presented in~\cite{Robson_2019}.} as blue dots in \textbf{Figure~\ref{fig: Final results plot}}.
\begin{figure}[ht!]
    \begin{center}
        \includegraphics[width=\textwidth]{images/Final_results_plot.pdf}
    \end{center}
    \caption{The blue dots represent the total GW signal of the simulated DWDs hosted by the $66,568$ galaxies of the estimated potential gravitational wave emitting galaxies, whereas the orange dots represent the signal considering only the $20,246$ of the GWGC after dropping the galaxies with incomplete information, and without considering the ZOA. In Both cases, the PSDs are binned in $\sim 8\times10^{-9}Hz$ large frequency bins, representing LISA's spectral resolution for an observational time $T_{obs}=4yr$. 
    Every dot represents the signal of a frequency bin, calculated by summing the Power Spectral Density (PSD) of every galaxy that it contained, using~\eqref{eq: total PSD}. 
...
The resulting calculated signal is about one order of magnitude smaller than the minimum detectable strain for LISA across the whole band.  
...
Moreover, the spectral shape of the resulting foreground is located at frequencies similar to the expected Galactic foreground ones, corresponding to the slight bump at the left of the lowest point in the graph (this is better shown in \textbf{Figure~\ref{fig: LISA sens curve with noises}}), but with a significantly lower amplitude due to the much larger average distances.
...
The main assumptions that we made to obtain the results are surely how we calculated the GW characteristic strain, and the way we integrate the signal coming from unresolved sources within the same frequency bin.
The GW signal calculated in~\eqref{eq: the strain we use} is the standard characteristic strain predicted from the General Relativity for an inspiraling binary system, and is therefore the most precise way to represent the DWDs signal.
