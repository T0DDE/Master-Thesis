...
In anticipation of the launch of a space-based gravitational-wave detector, the study of the expected signal from DWDs entirely relies on simulations.  
To this end, binary population synthesis codes have been developed as dedicated tools, whose accuracy benefits significantly from high-quality electromagnetic observations, especially in the era of large-scale surveys such as the {Sloan Digital Sky Survey \underline{\href{https://www.sdss.org/}{(SDSS)}}, the Zwicky Transient Facility \underline{\href{https://www.ztf.caltech.edu/}{(ZTF)}}, the Dark Energy Spectroscopic Instrument \underline{\href{https://www.desi.lbl.gov/}{(DESI)}}, and \underline{\href{https://sci.esa.int/web/euclid}{Euclid}}}.  
These observations not only provide complementary insights but also serve as crucial constraints to guide and validate the simulations, ensuring that the predicted populations are consistent with the systems already observed.  
In this chapter, we will first discuss population synthesis codes, then introduce in detail the \texttt{COSMIC} code used in this work. Finally, we will present the astrophysical surveys that we adopted as reference for the simulation of our sources.
...
\section{Stellar Population Synthesis Codes}
Generating a synthetic population of stars is a very complex task, that includes multiple steps, each involving important choices.
\textit{First}, we need to choose a starting point: we could start from the very beginning of star's formation and simulate all the process from the birth onward, or we could select a later phase in the star's evolution, shared from the most, in order to reduce unnecessary computational power and time consumption. 
If we want to simulate entire stellar populations choosing a starting point also implies selecting appropriate distributions for the main parameters that characterize the ``starting point population'', such as masses, metallicities, but also orbital parameters for the stars that are in binary systems, such as orbital period, distance, and eccentricity. 
\textit{Second}, we should choose how the stars will evolve from the starting point, and this involves the single star evolution but also the effects that interaction with other stars in binary systems have on it.
...
\begin{figure}[h!]
    ...
    \end{center}
    \caption{Hertzsprung–Russell (H–R) diagram showing the relationship between stellar luminosity (in solar units) and surface temperature (in Kelvin), that indicates the main stellar evolutionary stages: the main sequence, were most stars lie, the giant and supergiant branches, and the white dwarf region.
...
This allows to bypass the early phases of star formation, which are much less relevant to generate the sources of interest, while still capturing the essential evolutionary processes that lead to the formation of compact objects.
...
Also, in order to make population synthesis statistically robust a large enough number of stars of a certain type should be evolved in order to overcome stochastic noise (in particular, the Poisson noise for $n$ simulations of a particular type of star, implies an error that grows as $\sqrt{n}$).
...
These interactions can drastically alter the final outcomes, and are essential for modeling the formation of compact binaries that are potential GW targets for LISA. To efficiently model binary evolution within the framework of stellar population synthesis, the work of~\cite{BSE} extends the SSE formalism by introducing a set of prescriptions for binary interactions, and updating the treatment of processes such as Roche lobe overflow, common envelope evolution and coalescence by collision, leading to the development of the \textbf{Binary Star Evolution} (\textbf{BSE}) algorithm.
...
COSMIC's binary evolution is built upon BSE, incorporating extensive modifications in order to include updated physical prescriptions\footnote{{Beyond the various improvements in modeling astrophysical processes in binary evolution, the main strength of \texttt{COSMIC}, when compared with other BSE-based codes, lies in its efficient use of Monte Carlo techniques, which make large-scale binary population simulations highly efficient.  
From this point onward, whenever we refer to \textit{sampling}, we specifically mean this Monte Carlo approach.  
It is worth noting, however, that other recent codes exist, such as Stellar EVolution for N-body (\underline{\href{https://ascl.net/2206.019}{SEVN}},~\cite{SEVN}), which, instead of relying on the SSE and BSE algorithms, couples the PAdova and TRieste Stellar Evolution Code (PARSEC,~\cite{PARSEC}) stellar evolution tracks with up-to-date prescriptions for supernova explosions.}  
}.
It  includes all necessary tools to generate a population, from the generation of initial conditions, to scaling the simulated systems to full-size astrophysical populations.
The code is presented in~\cite{Breivik}, where it is described in full detail and used, as a proof of concept, to simulate the Galactic population of compact binaries and their associated GW signal.
...
The \textit{kstar values} (e.g.\ the number that represent a specific stellar type) and evolution stages are nearly identical to their BSE counterparts, and the exact references can be found in \textbf{Table~\ref{tab: kstar values}}. 
\begin{table}
    ...
    \end{center}
\end{table}
In order to generate a fixed population, COSMIC can be ran through a one-line command directly on the terminal, specifying a parameter file, the kstar values for the primary and secondary star, the maximum number of systems to evolve, every how many systems to check in, in order to track the distributions of the parameters, and how many processors to use.
The final output is in an \textit{hdf5} file containing several data frames, that keep track of various important quantities during the evolution: the total number of stars and total mass of the entire population, the number of binaries, the convergence, and so on. 
...
The parameter distributions of a fixed population of binary white dwarfs with a default metallicity value set at $0.020$ are shown in \textbf{Figure~\ref{fig: first fixed pop distributions}}.
\begin{figure}[ht!]
    \begin{center}
...
Once the convergence criteria are achieved, an astrophysical population can be sampled. 
...
population, $N_{astro, tot}$, to the total number of stars in the whole small-scale galaxy in which the fixed population is embedded, $N_{fixed,tot}$,
\begin{equation}
...
As we will now see, the chosen reference for our purpose is a catalog that reports many key galactic parameters in it, which will allow us to proceed using the method in~\eqref{eq: scale by mass}.



\section{{The} Gravitational Wave Galaxy Catalog ({GWGC})}
As we have seen, the goal of this work is to simulate the GW foreground produced by compact binaries in the \textit{local universe}, by generating the sources using COSMIC.
To replicate the existing, observed galaxies in the vicinity of the Milky Way and simulate their stellar content we rely on the dataset provided in~\cite{GWGC}, the \textbf{GWGC}.
This catalog includes a list of $53,255$ galaxies within $100Mpc$ from earth, containing information on sky position, distance, blue magnitude, major and minor diameters, position angle, and galaxy type, currently used for follow-up searches of electromagnetic counterparts from GW searches.
...
However, this is simply not practical because of the computational power and time it would require\footnote{A single fixed population can require up to $\sim4$h to be generated {on a commercial laptop with 12-core CPU}, and the GWGC contains $\sim50,000$ galaxies: generating a fixed population per each galaxy would require a computational time of $\sim20$yr.}, and therefore we should find a strategy to group them in a few, representative, categories.
...
\[
    m_{B,gal} - m_{B,\odot} = -2.5\log_{10}\left(\frac{L_{B,gal}}{L_{B,\odot}}\right),
\]
which, in stellar units, we get to the following conversion:
\begin{equation}
...
Although the GWGC denotes galaxy morphology using the~\cite{Vaculeurs} T-type scale, the luminosity-to-mass relations used in~\cite{Faber&Gallagher} are defined in terms of Hubble morphological classes.
...
Fortunately, a translation between T-type values and Hubble types is possible (\cite{De_Vaucouleurs_1974}), and thanks to the results in~\cite{Faber&Gallagher} we find the following correspondences:
\begin{table}[h!]
    \centering
    \begin{tabular}{ccc}
...
\end{table}
\vspace{3mm}\\
The luminosity resulting from~\eqref{eq: magnitude to luminosity}, expressed in units of solar blue luminosity $L_{B,\odot}$, can then be multiplied by the appropriate $M/L_B$ ratio, found in \textbf{Table~\ref{tab: mass luminosity conversions tab}}, to yield the total stellar mass for the galaxy
\begin{equation}
...
The study, based on a sample of over 50,000 star-forming galaxies observed by the Sloan Digital Sky Survey, spanning three orders of magnitude in stellar mass and a factor of ten in metallicity, reveals a tight correlation between the two, which can be written as follows:
\begin{equation}
...
\end{equation}
is the metallicity written in terms of the oxygen abundance\footnote{All the metallicity values with an apostrophe are intended as in these units.}.

\subsubsection{On metallicity units}
Now, in these units the sun's metallicity is $Z_\odot ' = 8.69$ ({\cite{AllendePrieto}}), whereas in the units used by COSMIC it is $Z_\odot=0.0134$ ({\cite{Asplund}})\footnote{All the metallicity values without the apostrophe are intended as in these units.}.
...
\end{equation}
where the ``$27.3$'' coefficient depends on the choice of the $Z_\odot$ value for which, as mentioned above, we infer the widely accepted value reported in~\cite{Asplund}.
...
This way, we can account for the significant impact of metallicity on stellar wind strength, remnant masses, and binary evolution outcomes, that greatly affects the expected GW signal.
