...
Before the launch of a space-based gravitational-wave detector, the study of the expected signal from DWDs relies entirely on simulations.
To this end, binary population synthesis codes have been developed as dedicated tools, whose accuracy benefits significantly from high-quality electromagnetic observations, especially in the era of large-scale surveys such as \underline{\href{https://www.sdss.org/}{SDSS}} and \underline{\href{https://www.ztf.caltech.edu/}{ZTF}}.
...
In this chapter we will discuss on population synthesis codes, to then introduce in detail the COSMIC code that we use in this work.
We then introduce the astrophysical survey that we used as a reference for the simulation of our sources.
...
COSMIC's binary evolution is built upon BSE, incorporating extensive modifications in order to include updated physical prescriptions\footnote{Other then the various improvements for astrophysical processes in binary evolution, the biggest strength of COSMIC, when compared with other BSE based codes, is the use of Monte Carlo to make the large binary population simulation extremely efficient.}.
...
\section{Gravitational Wave Galaxy Catalog}
...
However, this is simply not practical because of the computational power and time it would require\footnote{A single fixed population can require up to $\sim4$h to be generated, and the GWGC contains $\sim50,000$ galaxies: generating a fixed population per each galaxy would require a computational time of $\sim20$yr.}, and therefore we must find a strategy to group them in a few, representative, categories.
