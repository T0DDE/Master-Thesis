...
This allows to bypass the early phases of star formation, which are much less relevant to the GE sources of interest, while still capturing the essential evolutionary processes that lead to the formation of compact objects.
...
These interactions can drastically alter the final outcomes, and are essential for modeling the formation of compact binaries that are potential GW targets for LISA.
...
\section{\correzione{The COSMIC Code}}
...
COSMIC's binary evolution is built upon BSE, incorporating extensive modifications in order to include updated physical prescriptions\footnote{\correzione{Beyond the various improvements in modeling astrophysical processes in binary evolution, the main strength of \texttt{COSMIC}, when compared with other BSE-based codes, lies in its efficient use of Monte Carlo techniques, which make large-scale binary population simulations highly efficient.  
...
The \textit{kstar values} (e.g. the number that represent a specific stellar type) and evolution stages are nearly identical to their BSE counterparts, and the exact references can be found in \textbf{Table~\ref{tab: kstar values}} in the \textbf{Appendix}.
...
\section{\correzione{The} Gravitational Wave Galaxy Catalog}
...
However, this is simply not practical because of the computational power and time it would require\footnote{A single fixed population can require up to $\sim4$h to be generated \correzione{on a commercial laptop with 12-core CPU}, and the GWGC contains $\sim50,000$ galaxies: generating a fixed population per each galaxy would require a computational time of $\sim20$yr.}, and therefore we should find a strategy to group them in a few, representative, categories.
...
Now, in these units the sun's metallicity is $Z_\odot ' = 8.69$ (\correzione{\cite{AllendePrieto}}), whereas in the units used by COSMIC it is $Z_\odot=0.0134$ (\correzione{\cite{Asplund}})\footnote{All the metallicity values without the apostrophe are intended as in these units.}.
