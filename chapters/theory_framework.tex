...
\section{Double White Dwarfs \correzione{(DWDs)}}
...
This positive feedback process induces a very fast helium burning called \textit{helium flash}, that lifts the star back out of the degeneracy stage (\correzione{\cite{on_helium_flash}}).
...
These types of medium-to-low massive star's remnant, supported entirely by electron degeneracy pressure and slowly cooling over time, are what we call \textbf{white dwarfs} (\correzione{\cite{Bradt}}).

\subsubsection{Binary systems}
A great fraction of the white dwarfs in the universe are found DWDs systems, that show a wide range of orbital separations and periods, depending on their formation and evolutionary history (\correzione{\cite{evolution_compact_binary_wd_systems}}).  
In particular, in the Milky Way, DWDs are expected to be extremely common systems, many of which having orbital periods short enough to emit GWs in the milliHertz band (\correzione{\cite{Nelemans_2001}}).  
...
The nature of this transfer depends on the mass ratio: the transfer can be stable, leading to long-lived systems characterized by persistent mass accretion and bright electromagnetic counterparts, or it can become dynamically unstable, potentially resulting in the rapid merger of the two stars, which may produce a single more massive white dwarf, a neutron star, or even trigger a thermonuclear explosion if the critical conditions for a Type Ia supernova are met (\correzione{\cite{Das2025},~\cite{Shen_2009},~\cite{refId0},~\cite{Shen_2018}}).


\subsection{\correzione{The host galaxies}}
...
\correzione{The DWDs described in the previous section are massive rotating objects, and therefore they produce ripples in spacetime that propagate as waves (\cite{Einstein_1918}).
...
\begin{figure}[h]
    ...
    \end{tikzpicture}
    \caption{Effect of the two GW polarizations on a ring of free-falling particles, caused by a wave moving in the direction perpendicular to the page: blue and red dots represent two opposite phases of the wave.}
...
The most sensitive instrument capable of such measurements are the interferometric detectors, that are generally based on the same principle of the simple Michelson interferometer: by splitting a laser beam along two perpendicular arms and recombining it, they detect the small phase shifts induced by a passing GW.  
...
\subsection{\correzione{The} Laser Interferometer Space Antenna (\correzione{LISA})}
\correzione{Currently operating} detectors such as LIGO and Virgo (\correzione{\cite{PhysRevLett.116.061102},~\cite{Abbott_2019}}) have pioneered the technique we have just shown in the high-frequency band (\correzione{\cite{LISA_ref},~\cite{Virgo_cit}}).
The purpose for future space-based interferometers such as LISA is to extend it to the low-frequency regime (\correzione{approximately} within $0.1$mHz and $1$Hz), opening a new observational window on astrophysical and cosmological sources.
Indeed, space-based detectors such as LISA will be sensitive to DWDs with periods of a few hours down to a few minutes (\correzione{\cite{baker2019laserinterferometerspaceantenna}}). 
...
Between the binary systems, the interacting ones can have extremely short orbital periods ($\lesssim 10$ minutes) and are strong, nearly monochromatic GW sources over LISA's lifetime, which in this work is considered of $\sim4$yr (\correzione{\cite{baker2019laserinterferometerspaceantenna}}).  
Detached systems with periods in the mHz range are also important, as they provide long-lived, stable signals that can serve as calibration sources or ``verification binaries'' \correzione{(\cite{kupfer_2018},~\cite{Kupfer_2024})} for space-based detectors.
...
In LISA's case, the arms are not going to be physical structures, but $2.5\times10^6$km separations between three identical spacecrafts at the vertices of an equilateral triangle (\correzione{\cite{baker2019laserinterferometerspaceantenna}}).
...
The spacecrafts will trail by about $20^{\circ}$, with the triangular constellation inclined by $60^{\circ}$ to the ecliptic plane, following the earth around the sun (\correzione{\cite{baker2019laserinterferometerspaceantenna}}).
...
LISA's frequency band roughly covers the low range $0.1~\mathrm{mHz} \lesssim f \lesssim 1~\mathrm{Hz}$ (compared to LIGO's frequency of 10 Hz to 10 kHz (\correzione{\cite{Martynov_2016}}), corresponding to objects in much wider orbits and potentially much heavier than those that LIGO is searching for, opening up the to the detection of a wider range of GW sources. 
In this frequency range also reside most DWDs.  
In the low frequencies, the range is limited by the acceleration noise on the test masses: even in space, the ``free'' test masses are not completely isolated, but could be influenced by solar radiation and winds, and the ``drag-free'' micro-propellers that compensate these disturbances make noise under the $0.1~\mathrm{mHz}$ (\correzione{\cite{baker2019laserinterferometerspaceantenna}}). 
...
At the high frequencies, the main limits are due to the arms length, which make LISA unable to detect waves much shorter than that scale, and the power limits of the laser, which makes the photon count Poissonian noise more relevant at high frequencies (\correzione{\cite{baker2019laserinterferometerspaceantenna}}).  
LISA’s sensitivity curve combines instrumental noise and the astrophysical foreground, including Galactic foreground and Galactic binary systems, as shown in \textbf{Figure~\ref{fig: LISA sens curve with noises}} (\correzione{\cite{LISA_ref}}). 
\begin{figure}
    \begin{center}
...
The source for this image can be found \underline{\textit{\href{https://ar5iv.labs.arxiv.org/html/1803.01944}{here}}} (\correzione{\cite{LISA_ref}})}\label{fig: LISA sens curve with noises}
\end{figure}
...
\correzione{From a scientific perspective, double white dwarfs (DWDs) provide an excellent laboratory for gravitational-wave studies:  
\begin{itemize}
...
% \end{itemize}
% We expect the Galactic binaries to generate a confusion foreground that is able to impact the LISA sensitivity curve, mostly due to their proximity and large number \correzione{(\cite{osti_6282329},~\cite{Nelemans_2001},~\cite{LISA_ref}}.
% The purpose of this work, however, is to discover if the \textit{extra-galactic} DWDs, despite their much larger distance, \correzione{contribute significantly} to generate an overall detectable stochastic GW foreground in the LISA band.
