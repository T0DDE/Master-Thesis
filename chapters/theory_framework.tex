...
A great fraction of the white dwarfs in the universe are found DWDs systems, that show a wide range of orbital separations and periods, depending on their formation and evolutionary history.  
In particular, in the Milky Way, DWDs are expected to be extremely common systems, many of which having orbital periods short enough to emit GWs in the milliHertz band.  
...
The nature of this transfer depends on the mass ratio: the transfer can be stable, leading to long-lived systems characterized by persistent mass accretion and bright electromagnetic counterparts, or it can become dynamically unstable, potentially resulting in the rapid merger of the two stars, which may produce a single more massive white dwarf, a neutron star, or even trigger a thermonuclear explosion if the critical conditions for a Type Ia supernova are met\footnote{More on white dwarfs can be found in~\cite{Shen_2009} and~\cite{refId0}, and in~\cite{Shen_2018} while in~\cite{Das2025} are more details of the Supernova detonation from white dwarfs}.
...
The purpose for future space-based interferometers such as LISA is to extend it to the low-frequency regime (within $0.1$mHz and $1$Hz), opening a new observational window on astrophysical and cosmological sources.
...
Between the binary systems, the interacting ones can have extremely short orbital periods ($\lesssim 10$ minutes) and are strong, nearly monochromatic GW sources over LISA's lifetime, which in this work is considered of $\sim4$yr.  
Detached systems with periods in the mHz range are also important, as they provide long-lived, stable signals that can serve as calibration sources or ``verification binaries''\footnote{\cite{kupfer_2018},~\cite{Kupfer_2024}} for space-based detectors.
...
LISA's frequency band roughly covers the low range $0.1~\mathrm{mHz} \lesssim f \lesssim 1~\mathrm{Hz}$ (compared to LIGO's frequency of 10 Hz to 1000 Hz), corresponding to objects in much wider orbits and potentially much heavier than those that LIGO is searching for, opening up the to the detection of a wider range of GW sources. 
...
LISA’s sensitivity curve combines instrumental noise and the astrophysical foreground, including Galactic foreground and Galactic binary systems, as shown in \textbf{Figure~\ref{fig: LISA sens curve with noises}}. 
\begin{figure}
    \begin{center}
...
As shown in \textbf{Figure~\ref{fig: LISA sens curve with noises}}, we expect the Galactic binaries to generate a confusion foreground that is able to shape the LISA sensitivity curve, mostly due to their proximity and large number\footnote{\cite{osti_6282329}, \cite{Nelemans_2001}}.
The purpose of this work, however, is to discover if the \textit{extra-galactic} binaries, despite their much grater distance, are sufficiently numerous to generate an overall detectable stochastic GW foreground in the LISA band.
In particular, from a scientific perspective, DWDs offer a rich laboratory:  
    \begin{itemize}
        \item The DWDs binary systems typically are in compact orbits, and the corresponding frequencies fall in LISA's band; 
        ...
        When the number of non-resolvable sources is big, their signals superpose stochastically, generating a foreground; 
        \item We have found a simple and predictable shape for their signal in the circular orbit approximation, in~\eqref{eq: the strain we use}, which is possible to simulate, and allows to generate coherent synthetic populations with astrophysical models; 
